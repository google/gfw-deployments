...
Description:
For all users in an Apps Domain, alters the primary calendar's domain
wide sharing to "freebusy" only

Usage: update_calendar_sharing.py -k <consumer key> -s <consumer secret> -u <admin user> -p
              <admin password> [-t <# threads> -r <# retries> -f <# max failures>]
...
Options:
  ...
  --apply             If present, changes will be applied, otherwise
                      will run in dry run mode with no changes made

Typical Usage:
  - Run first in "Dry Run Mode". Here we use 10 threads and retry a max of 5 times:
     ./update_calendar_sharing.py -k mdauphinee.info -s "flkjlkjsdafpoipojlkjlkjF"
                                  -u administrator@mdauphinee.info -p mypass -r 5 -t 10
  - If looks appropriate, run with --apply to apply changes
     ./update_calendar_sharing.py -k mdauphinee.info -s "flkjlkjsdafpoipojlkjlkjF"
                                  -u administrator@mdauphinee.info -p mypass -r 5 -t 10 --apply
...
class CalendarWorker(threading.Thread):
  ...
  def _ProcessInstruction(self, task):
    ...
    if (task.GetIsCalDomainShared() and
      ...
      if self.apply:
        ...
        logging.info("[%s] Would have added sharing for [%s]",
                     self.name, task.GetUserName())
    elif (task.GetIsCalDomainShared() and
          not self._ExistingCalFreeBusyShared(connection)):
      if self.apply:
        ...
        logging.info("[%s] Would have Removing sharing for [%s]",
                     self.name, task.GetUserName())
        logging.info("[%s] Would have Adding sharing for [%s]",
                     self.name, task.GetUserName())
    else:
      if self.apply:
        ...
        logging.info("[%s] Would have done nothing for user: [%s]",
                     self.name, task.GetUserName())
  ...
  def _ExistingCalFreeBusyShared(self, connection):
    """Method to check to see if the Calendar is already shared for freebusy."""

    feed = connection.GetCalendarAclFeed()

    for acl_entry in feed.entry:
      if acl_entry.scope.type == 'domain':
        if acl_entry.title.text == 'freebusy':
          return True
...
def main():
  ...
  # Set up logging
  log_filename = ('update_calendar_sharing_%s.log' %
